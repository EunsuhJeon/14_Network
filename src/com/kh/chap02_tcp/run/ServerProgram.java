...
public class ServerProgram {
	 ...
	 * - 데이터를 교환하기에 앞서서 서버, 클라이언트 연결되어있어야 함! (서버가 먼저 실행되어 클라이언트의 요청 기다릴 것)
	 * - 신뢰성 있는 데이터 전달 가능
	 * 
	 * * Socket
	 * - 프로세스 간의 통신을 담당
	...
	public static void main(String[] args) {
		...
		// 1) 포트 번호 지정 (서버 측에서 몇 번 포트로 통로를 열겠는지를 의미함)
		int port = 3000;
		
		ServerSocket server = null;
		try {
			// 2) ServerSocket 객체 생성 시 포트 결합 (Bind) 	(ServerSocket은 클라이언트의 연결 요청을 받아줄 용도의 소켓)
			...
			while(true) {
				// 7) 스트림을 통해 읽고 쓰기
				// 클라이언트로부터 전달된 메세지가 있을 경우 서버 측 읽어들일것! (입력)
				...
				pw.flush(); // 현재 스트림에 있는 데이터를 강제로 내보내는 메소드 (close메소드 호출되면 flush)
			}
